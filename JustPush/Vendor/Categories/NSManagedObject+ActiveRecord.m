...
+ (NSManagedObjectContext *)defaultContext {
    return [(JPAppDelegate*)[NSApplication sharedApplication].delegate managedObjectContext];
}
...
+ (instancetype)findOrCreate:(NSDictionary *)properties inContext:(NSManagedObjectContext *)context {
    NSManagedObject *existing = [self where:properties inContext:context][0];
...
+ (id)create:(NSDictionary *)attributes inContext:(NSManagedObjectContext *)context {
    if (![attributes exists]) return nil;
